...
export interface DatePickerProps {
	...
	mode?: DatePickerMode;
	enableTime?: boolean;
	// En `mode="range"` Flatpickr maneja arrays de longitud variable (0..2).
...
const inputBase =
	'w-full min-h-(--touch-min) rounded-xl border border-white/[0.1] bg-slate-950/45 pl-10 pr-4 py-2.5 text-sm text-slate-50 shadow-[inset_0_1px_2px_rgba(0,0,0,0.25)] outline-none ring-0 backdrop-blur-sm transition placeholder:text-slate-500 focus:border-teal-400/70 focus:ring-2 focus:ring-teal-400/25 disabled:cursor-not-allowed disabled:opacity-60';
...
export default function DatePicker({
	...
	const handleChange = useCallback(
		...
		[onChange, mode, nativeError]
	);

	// En range, evitamos pasar defaultDate cuando el rango está "incompleto"
	// (1 sola fecha) para no forzar reinicializaciones del picker durante la selección.
	const defaultDateForFlatpickr = useMemo(() => {
		if (value == null || value === '') return undefined;
		if (mode === 'range' && Array.isArray(value)) {
			return value.length === 2 ? value : undefined;
		}
		return Array.isArray(value) ? value : (value as string | Date);
	}, [value, mode]);
	...
	const options = useMemo(() => {
		const opts: Record<string, unknown> = {
			...
			altFormat,
			defaultDate: defaultDateForFlatpickr,
		...
		return opts;
	}, [mode, dateFormat, altFormat, minDate, maxDate, enableTime, enabledDates, disabledDates, useAltInput, defaultDateForFlatpickr]);
