  ...
  return (
    <div className={clsx('space-y-1.5', className)}>
      ...
      >
        <div
          className={clsx(
            'group relative flex min-h-[7.5rem] w-full flex-col items-center justify-center overflow-hidden rounded-xl border border-dashed border-white/15 bg-slate-900/40 text-center shadow-[inset_0_1px_2px_rgba(0,0,0,0.15)] transition',
            ...
            aspect === 'square' && 'mx-auto min-h-[7.5rem] max-w-[11rem] sm:h-36 sm:min-h-0 sm:w-36 sm:max-w-none',
            aspect === 'wide' && 'min-h-[8.5rem]'
          )}
        >
          <input
            ref={inputRef}
            ...
            aria-labelledby={label ? labelTextId : undefined}
            aria-label={
              label ? undefined : `Subir imagen: ${emptyDescription}`
            }
            className={clsx(
          ...
          <div className="pointer-events-none absolute inset-0 z-10 flex flex-col items-center justify-center px-3 py-6">
            ...
            >
              <span className="rounded-full border border-white/10 bg-slate-950/60 px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-teal-200/90">
                {hasNewFile ? 'Nuevo archivo' : hasRemoteOnly ? 'Actual en servidor' : 'Sin archivo'}
              </span>
              ...
              {hasRemoteOnly && !hasNewFile && existingUrl && (
                <p
                  ...
                  title={existingUrl}
                >
                  {existingUrl.length > 96 ? `${existingUrl.slice(0, 96)}…` : existingUrl}
                </p>
              )}
            </div>
          </div>
