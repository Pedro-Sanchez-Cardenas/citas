...
> = {
	...
	products: {
		...
		description: 'Registra los productos que usas y vendes para controlar inventario y costos.',
		icon: '🧴',
	},
};
...
export function BusinessOnboardingPanel({ setup, isLoading }: BusinessOnboardingPanelProps) {
	...
	return (
		<section className="rounded-2xl border border-white/10 bg-slate-900/70 p-5 shadow-(--shadow-modal) backdrop-blur-2xl lg:col-span-2">
			<div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
				<div>
					...
					<p className="mt-1 text-sm text-slate-400">
						Completa estos pasos clave para dejar listo tu panel y empezar a agendar
						citas en minutos.
					</p>
				</div>
						...
						<div className="mb-2 flex items-center justify-between gap-2">
							<div>
								...
								<p className="mt-0.5 text-xs text-slate-300">
									Sigue estos pasos en orden sugerido. Puedes saltar entre ellos
									cuando quieras.
								</p>
							</div>
							<span className="shrink-0 text-[11px] text-slate-400">
								Paso {steps.findIndex((s) => s.key === activeStep.key) + 1} de{' '}
								{totalSteps}
							</span>
						</div>
						{activeStep && (
							...
							<div
								...
								className={`flex flex-col justify-between gap-3 rounded-xl border px-3.5 py-3 text-xs transition-shadow sm:text-sm ${
									isActive
										...
										: 'border-white/8 bg-slate-950/35 hover:border-white/12 hover:bg-slate-950/50'
								}`}
							>
									...
									<div
										className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-2xl text-base ${
											isDone
												...
												: 'bg-white/6 text-slate-200 ring-1 ring-white/6'
										}`}
										aria-hidden
									>
										{config.icon}
