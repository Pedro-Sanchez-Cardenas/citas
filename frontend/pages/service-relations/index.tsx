...
export default function ServiceRelationsPage() {
    ...
    async function loadRelations() {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          setProfessionalIds(new Set(prosList.map((p) => p.id)));
          ...
          matList.forEach((m) => {
            const quantity = m.pivot?.quantity != null ? Number(m.pivot.quantity) : 0;
  ...
  const handleChangeMaterialQuantity = (id: number, value: string | number) => {
    setMaterials((prev) => {
      const current = prev[id] ?? { checked: false, quantity: 0 };
      return { ...prev, [id]: { ...current, quantity: value === '' ? '' : Number(value) } };
    });
  };
  ...
  const handleSaveMaterials = async () => {
    ...
    try {
      ...
      await syncServiceMaterials(selectedServiceId, { materials: materialsPayload });
    } catch (err) {
      const ax = err as AxiosError<{ message?: string }>;
      setError(
        ax?.response?.data?.message ||
        ...
        <div>
          ...
          {selectedService && (
            <p className="mt-1 text-[11px] text-slate-500">
              Código: {selectedService.code} • Duración: {selectedService.duration_minutes} min
            </p>
          )}
        </div>
          ...
          <div className="rounded-2xl border border-slate-800/80 bg-slate-950/80 p-4 shadow-[0_10px_30px_rgba(15,23,42,0.85)]">
            ...
            <div className="max-h-80 space-y-2 overflow-y-auto pr-1">
              {products.map((product) => {
                const entry = materials[product.id] ?? { checked: false, quantity: '' as const };
                return (
                  <div
                    key={product.id}
                    className="flex items-center justify-between gap-3 rounded-xl border border-slate-800/80 bg-slate-950/80 px-3 py-2 text-xs text-slate-200"
                    ...
                    <div className="w-24">
                      <Input
                        ...
                        onChange={(e: ChangeEvent<HTMLInputElement>) =>
                          handleChangeMaterialQuantity(product.id, e.target.value)
                        }
                        placeholder="0"
