  ...
  const handleSubmitCategory = async (formData: { name: string; description?: string | null }) => {
    ...
    try {
      if (selectedCategory?.id) {
        ...
        setCategories((prev) =>
          prev.map((cat) => (cat.id === selectedCategory.id ? (updated as ServiceCategoryRecord) ?? cat : cat))
        );
      } else {
        const created = await createServiceCategory(formData as Record<string, unknown>);
